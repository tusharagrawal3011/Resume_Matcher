...
export class MatchResumesUseCase {
  ...
  async execute(input: MatchResumesInput): Promise<MatchResumesOutput> {
    ...
    const tasks: Promise<CandidateMatch | null>[] = retrieved.map((candidate) =>
      limiter.run(async () => {
    ...
    const resolved: (CandidateMatch | null)[] = await Promise.all(tasks);

    // Type-safe filtering
    const matches: CandidateMatch[] = resolved.filter((m): m is CandidateMatch => m !== null);
